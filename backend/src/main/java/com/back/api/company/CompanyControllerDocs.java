...
public interface CompanyControllerDocs {
  ...
  ResponseEntity<Page<CompanySummaryResponse>> getAllCompanies(
          @Parameter(description = "페이지 번호 (0부터 시작)", example = "0") int page,
          @Parameter(description = "페이지 크기", example = "12") int size,
          @Parameter(description = "정렬 기준 (name 또는 score)", example = "score") String sort
  );

  @Operation(summary = "기업 검색", description = "기업명에 검색어가 포함된 기업 목록을 반환합니다.")
  ResponseEntity<List<CompanySummaryResponse>> searchCompanies(
          @Parameter(description = "검색할 기업명 또는 키워드", example = "Apple") String keyword
  );

  @Operation(summary = "기업 상세 정보 조회", description = "특정 기업의 기본 정보, 재무 건전성 점수, 최근 재무제표 내역을 조회합니다.")
  ResponseEntity<CompanyDetailResponse> getCompanyDetail(
          @Parameter(description = "기업 티커 (예: AAPL)", example = "AAPL") String ticker
  );

  @Operation(summary = "주가 차트 데이터 조회", description = "특정 기업의 주가 데이터를 조회합니다.")
  ResponseEntity<List<StockHistoryResponse>> getCompanyChart(
          @Parameter(description = "기업 티커 (예: AAPL)", example = "AAPL") String ticker
  );
}