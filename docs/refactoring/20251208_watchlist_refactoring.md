...
### 2.1. 계층 간 책임 불분명 (Fat Controller)
...
### 3.1. Layered Architecture 적용 (Service 계층 도입)
...
### 4.1. 성능 지표 비교 (관심 종목 10개 기준)

| 구분 | 변경 전 (AS-IS) | 변경 후 (TO-BE) | 개선 효과 |
| :--- | :---: | :---: | :---: |
| **쿼리 실행 횟수** | 11회 (1 + 10) | **1회** | **91% 감소** |
| **트랜잭션 범위** | Controller | Service | 데이터 정합성 보장 |
...
이번 리팩토링을 통해 **"작동하는 코드"와 "좋은 코드"의 차이**를 명확히 이해했습니다. 특히 JPA 사용 시 연관 관계 매핑만으로는 성능 문제가 발생할 수 있음을 인지하고, `Fetch Join`을 적재적소에 활용하는 능력을 길렀습니다. 향후 대용량 트래픽 상황에서도 DB 부하를 최소화할 수 있는 기반을 마련했습니다.
