...
public class ScoreEvaluator {
  ...
  public ScoreEvaluationResultDto evaluate(Map<ScoreCategory, Integer> componentScores, int penalty, FinancialStatement fs) {
    ...
    // 저점 매수 여부 판단
    boolean isOpportunity = isUnderestimated(valuation, fs.getTotalEquity());

    return new ScoreEvaluationResultDto(
  ...
  // 저점 매수 여부 판단 헬퍼 (자본잠식이 없고, 밸류에이션 점수가 높을 때)
  private boolean isUnderestimated(int valuation, BigDecimal equity) {
    return valuation >= OPPORTUNITY_VALUATION_THRESHOLD && equity.compareTo(BigDecimal.ZERO) > 0;
  }
