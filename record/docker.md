...
## --build를 붙이는 이유
  ...
  - MariaDB / Redis (남이 만든 것) image: mariadb:10.6 처럼 설정했다.
  이건 이미 누군가가 잘 만들어둔 완성품(이미지)을 인터넷(Docker Hub)에서 다운로드만 받아서 실행하면 된다. 따라서 빌드(Build) 과정이 필요 없다.

  - Spring Boot는 내가 만든 것. build: ./BACK/companyvalue 처럼 설정했다. 이건 다운로드 받는 게 아니라, 내 컴퓨터에 있는 JAR 파일(코드)을 가지고 즉석에서 이미지를 "제조(Build)" 해야 한다. --build 옵션은 "기존에 만들어둔 게 있더라도 무시하고, 최신 코드(JAR)를 넣어서 새로 제조하라는 뜻이다.
 ...
 * 특징
  - 실행 중인 프로그램(Spring Boot, DB, Redis)만 종료됩니다.
