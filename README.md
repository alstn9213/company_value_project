...
Demo Video: `핵심 기능 시연 영상 유튜브 링크` (추후 업데이트 예정)  

Note: "본 서비스는 GCP Free Tier 인스턴스와 무료 주식 데이터 API를 사용하여 구축되었습니다. 동시 접속 시 API 호출 제한(분당 5회)으로 인해 데이터 로딩이 지연될 수 있습니다."
...
| **투자 부적격(과락)** | 자본 잠식 상태 또는 부채비율 기준치 초과(업종별 상이) 초과 | **즉시 F등급 (0점)** |
